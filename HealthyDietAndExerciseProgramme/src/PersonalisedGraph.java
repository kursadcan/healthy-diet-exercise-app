...
public class PersonalisedGraph {
    ...
    class GraphDrawer extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            // Displaying what the graph values are 
            g2d.setColor(Color.MAGENTA);
            g2d.drawString("Weights(kg)", 70, 10);
            g2d.drawString("Dates(dd.mm.yy)", 1805, 905);
            
            double heightCoefficient =0;   // Height coefficient to adjust the intervals
            double[] weights=new double[progressDataRepo.size()];  // Array for ordered weights 
            double[] tempOfWeights=new double[progressDataRepo.size()];  // Array for unsorted weights 
            double[] heightOfWeights=new double[progressDataRepo.size()];  // Array for keeping the heights of the data to draw the graph line and perpendicular lines
            for (int i = 0; i < weights.length; i++) {
            	weights[i]=progressDataRepo.get(i).getUserWeight();	
            	tempOfWeights[i]=progressDataRepo.get(i).getUserWeight();
			}
            
            // Deciding the heightCoefficient according to the change of weight
            heightCoefficient=(endY-unitY)/((getMaxValue(weights)-(getMaxValue(weights)%10))+20-(getMinValue(weights)-(getMinValue(weights)%10)));
           
            // Keeping the height of the data to draw the line of change and perpendicular lines
            for (int i = 0; i < weights.length; i++) {
            	heightOfWeights[i]=endY-(unitY+heightCoefficient*(Math.abs((getMinValue(weights)-(getMinValue(weights)%10))-weights[i])));
			}
            
            insertionSort(weights); // Ordering weights 
            double leastWeight=weights[0];  // Keeping least weight
           
            // Drawing vertical gray lines
            ...
            // Determining the maximum height for the upper limit of the graph
            ...
            // Writing date information on the edges
            ...
            // Writing weight information on the edges
            ...
            // Showing numbers above the graph line
            g2d.setColor(Color.BLACK);
